...
#| = : = : = : = : = : = : = : = : = : = : = : = : = : = : = : = : = : = : = :  |    
#|{>/------------------------------------------------------------------------\<}| 
#|: | Script Name:	CollectTopCPUQueriesInPeakHour.sql							| 
#|: | Author	 :	Prakash Heda												| 
#|: | Email		 :	prakash@sqlfeatures.com	 Blog: http://www.sqlfeatures.com	|
#|: | Description:	This script collect 2 snapshots of top queries taxing		|
#|: | 				sql server CPU's within 5 min								|
...
use tempdb
go
truncate table dbastuff..CollectTopCPUQueries
go
...
select 
	servername,runtime,max(DBName) as DBName,max(QueryExecuted) as QueryExecuted,(sum(total_worker_time)/sum(execution_count))/@ConvertMiliSeconds as AvgCPUTime,sum(execution_count) as execution_count,query_hash, max(ObjectName) as ObjectName
...
if @debugFlg=1  select * from ##FindTopCPUQueries_set2
--where QueryExecuted like 'select TOP 300%'
order by AvgCPUTime desc


insert into dbastuff..CollectTopCPUQueries
...
where QueryStats.query_hash IN 
(
	select query_hash from dbastuff..CollectTopCPUQueries
)
ORDER BY total_worker_time/execution_count DESC;
